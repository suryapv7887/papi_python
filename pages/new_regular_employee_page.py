...
class NEW_REGULAR_EMPLOYEE:
    ...
    def click_add_employee(self):
        ...
        return NEW_REGULAR_EMPLOYEE(self.driver)

    def is_displayed(self):
        return self.helpers.wait_for_element(LOC.ADD_EMPLOYEE_FORM, condition="visible", timeout=10)

    def fill_basic_info(self, first, last, email, mobile,linkedin):
    ...
    def fill_address_details(self, present_address, permanent_address=None, same_as_present=False):
    ...
    def update_insurance_details(self,Insurance_required=False, dependent_data=None):
        if Insurance_required:
            ...
            if dependent_data:
                ...
                self.helpers.wait_and_click_dropdown_option(LOC.DEPENDENT_RELATIONSHIP_DROPDOWN, LOC.DEPENDENT_RELATIONSHIP_DROPDOWN_OPTIONS,
                                                            dependent_data["relationship"])
                self.helpers.send_keys(LOC.DEPENDENT_DOB, dependent_data["dob"])
        else:
            self.helpers.click(LOC.INSURANCE_NO)

    # def update_PF_details(self,PF_applicable=False, PF_data=None):
    #     if PF_applicable:
    #         self.helpers.click(LOC.INSURANCE_YES)
    #         self.helpers.click(LOC.ADD_DEPENDENT_BUTTON)
    #         if PF_applicable:
